...
class TrackLoss(Callback):
    ...
    def __init__(self, save_model=False, path=None, model_name=None):
        if save_model:
          if not path or not model_name:
            raise ValueError("you want to save but path or model name is None")
    ...
    def after_validate(self):
        ...
        if self.cur_model_val_loss < self.best_valid_loss:
          ...
          if self.save_model:
            torch.save(self.model.state_dict(), f"{self.path}/{self.model_name}_best_valid.pth")
          else: 
    ...
    def after_epoch(self):
        ...
        if self.save_model:
          if self.epoch > 1: # don't save the first epoch to save memory
            torch.save(self.model.state_dict(), f"{self.path}/{self.model_name}_epoch{self.epoch}.pth") 
...
class ProgressBar(Callback):
  def before_epoch(self):
    self.learner.pbar = tqdm(total=len(self.dls.train_dl), position=0, desc=f"epoch {self.epoch}", leave=True, file=sys.stdout)
